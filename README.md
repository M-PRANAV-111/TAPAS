...
/login offers public citizen entry and two Use Demo Account buttons. /officer composes local weather/map/resources with alert and occupational navigation; /authority adds a national sampled Heat Index comparison and local drill-down. The shared location and date persist in the URL. src/lib/auth/demoAuth.ts holds replaceable public demo account config and a localStorage role session. Guards, reload persistence, role switching and logout are frontend demonstrations only. Never place private data behind these guards.
