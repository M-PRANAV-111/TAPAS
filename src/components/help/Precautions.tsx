...
export function Precautions({ riskLevel, selectedDate, heatIndex }: { riskLevel?: RiskLevel | null; selectedDate?: string; heatIndex?:number|null }) {
  ...
  return (
    <section aria-labelledby="precautions-heading" className="rounded-lg border border-border bg-white p-3 sm:p-4">
      ...
      <label className="mt-3 flex flex-wrap items-center gap-2 text-xs font-medium">Advice for<select className="min-h-11 max-w-full rounded-md border border-border bg-white px-3 text-sm" value={audience} onChange={event => setAudience(event.target.value as GuidanceAudience)}>
