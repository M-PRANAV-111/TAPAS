...
// Standard OSM raster tiles (free, no API key) darkened entirely with MapLibre's
// own raster paint properties — CARTO's free anonymous dark tiles now require a
// key, and a CSS canvas-invert filter would also invert the risk-colour layers
// that share the same WebGL canvas, so this stays the only key-free path.
const BASE_STYLE: StyleSpecification = {version:8,sources:{osm:{type:'raster',tiles:[process.env.NEXT_PUBLIC_MAP_TILE_URL || 'https://tile.openstreetmap.org/{z}/{x}/{y}.png'],tileSize:256,maxzoom:19,attribution:'&copy; <a href="https://www.openstreetmap.org/copyright">OpenStreetMap</a> contributors'}},layers:[{id:'bg',type:'background',paint:{'background-color':'#0E0B08'}},{id:'osm',type:'raster',source:'osm',paint:{'raster-brightness-max':0.28,'raster-contrast':0.3,'raster-saturation':-0.6}}]}
...
export default function HeatMap({geojson,wards,selectedWardId,onWardSelect,location,resources=[],selectedResourceId,onResourceSelect,className,selectedDate}:HeatMapProps) {
  ...
  useEffect(()=>{
    ...
    const marker=new maplibregl.Marker({color:'#EA762B'}).setLngLat([location.longitude,location.latitude]).addTo(map)
  ...
  useEffect(()=>{
    ...
    if(!map.getSource('help')) {
      ...
      map.addLayer({id:'help-clusters',type:'circle',source:'help',filter:['has','point_count'],paint:{'circle-color':'#2E7D8C','circle-radius':17,'circle-stroke-color':'#F4E4CC','circle-stroke-width':2}})
      map.addLayer({id:'help-points',type:'circle',source:'help',filter:['!', ['has','point_count']],paint:{'circle-color':['match',['get','category'],'water','#3B82C4','medical','#9B6BC9','#2E7D8C'],'circle-radius':['case',['==',['get','id'],selectedResourceId ?? ''],10,7],'circle-stroke-color':'#F4E4CC','circle-stroke-width':2}})
    }
