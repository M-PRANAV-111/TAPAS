...
export function ThermalLayer({map,date}:{map:MapLibreMap|null;date:string}){
 ...
 useEffect(()=>{
  ...
  if(!map.getSource('thermal-points')){
   ...
   map.addLayer({id:'thermal-circles',type:'circle',source:'thermal-points',paint:{'circle-radius':['interpolate',['linear'],['zoom'],3,7,6,12,12,19],'circle-color':['get','color'],'circle-opacity':.85,'circle-stroke-color':'#FFFFFF','circle-stroke-width':1.5}})
  }
  const features=points.map(p=>({type:'Feature' as const,geometry:{type:'Point' as const,coordinates:[p.longitude,p.latitude]},properties:{id:p.id,color:metric==='heatIndex'?(heatIndexBand(p.heatIndex)?.color??'#64748B'):p.temperature===null?'#64748B':p.temperature>=40?'#641E16':p.temperature>=35?'#C0392B':p.temperature>=30?'#E67E22':p.temperature>=25?'#D4AC0D':'#2471A3'}}))
 ...
 return <div ref={overlay} className="relative z-10 shrink-0 border-t bg-white p-3" data-testid="heat-layer" data-point-count={points.length} data-status={status}><div className="flex flex-wrap items-center justify-between gap-x-3"><h2 className="text-xs font-semibold">{points.length} weather-model points</h2><label className="flex min-h-11 items-center gap-2 text-xs">Heat metric<select aria-label="Heat metric" className="min-h-11 max-w-[140px] rounded border bg-white p-2" value={metric} onChange={e=>setMetric(e.target.value as typeof metric)}><option value="heatIndex">Heat Index</option><option value="temperature">Air temperature</option></select></label></div><DataProvenance status={status} source={data?.provenance.source} timestamp={data?.provenance.originalAt} note={data?.provenance.status==='snapshot'?'Recorded model sample; original dates are shown. Live refresh pending or unavailable.':query.isFetching?'Refreshing the selected viewport…':undefined}/><p className="mt-1 text-[11px] tapas-subtext">{metric==='heatIndex'?'Gold: caution · orange: extreme caution · red: danger · dark red: extreme danger · grey: HI unavailable. Shade/light wind; not an official warning.':'Blue <25°C · gold 25–30°C · orange 30–35°C · red 35–40°C · dark red ≥40°C. Temperature colours are not official risk categories.'}</p>{!points.length?<p role="status" className="mt-1 text-xs">No live, cached or recorded weather points for this viewport. Zoom out to the recorded India grid.</p>:null}<button className="min-h-11 text-xs underline" onClick={()=>{if(map&&points.length){const b=new maplibregl.LngLatBounds();points.forEach(p=>b.extend([p.longitude,p.latitude]));map.fitBounds(b,{padding:{top:110,bottom:25,left:25,right:25},maxZoom:10,duration:0})}}}>Fit returned model points</button><details className="text-xs"><summary className="flex min-h-11 cursor-pointer items-center py-1">Inspect model points</summary><ul className="max-h-28 overflow-y-auto">{points.map(p=><li key={p.id}><button className="min-h-11 text-left underline" onClick={()=>select(p)}>{p.name}: {formatTemp(p.temperature)}, HI {formatHeatIndex(p.heatIndex)} · {formatDateTime(p.time)}</button></li>)}</ul></details></div>
}
