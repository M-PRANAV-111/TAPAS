...
interface SheetContentProps
  ...
  /** Suppress the built-in close when the content already provides one. */
  hideClose?: boolean
}
...
>(({ side = 'right', className, children, hideClose = false, ...props }, ref) => (
  <SheetPortal>
    ...
    >
      {children}
      {hideClose ? null : (
        <SheetPrimitive.Close className="absolute right-2 top-2 flex h-11 w-11 items-center justify-center rounded-sm bg-white opacity-90 transition-opacity hover:opacity-100 focus:outline-none">
          <X className="h-4 w-4" />
...
>(({ className, ...props }, ref) => (
  <SheetPrimitive.Description
    ref={ref}
    className={cn('text-xs tapas-subtext', className)}
    {...props}
  />
))
